...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("tang-core-test");
        ...
        sstream << "tang" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with tang-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing tang-core " << STELLAR_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "tang-core can run tests concurrently");
    session.cli(cli);
