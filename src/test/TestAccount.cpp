...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = tang::loadAccount(ls, getPublicKey());
        if (entry)
        {
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerState ls(mApp.getLedgerStateRoot());
        auto entry = tang::loadAccount(ls, publicKey);
        if (entry)
        {
    ...
    {
        LedgerState ls(mApp.getLedgerStateRoot());
        auto destAfter = tang::loadAccount(ls, publicKey);
    ...
    {
        LedgerState ls(mApp.getLedgerStateRoot());
        REQUIRE(tang::loadAccount(ls, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerState ls(mApp.getLedgerStateRoot());
    REQUIRE(tang::loadAccount(ls, into));
    REQUIRE(!tang::loadAccount(ls, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    LedgerState ls(mApp.getLedgerStateRoot());
    auto data = tang::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerState ls(mApp.getLedgerStateRoot());
        auto toAccountEntry = tang::loadAccount(ls, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerState ls(mApp.getLedgerStateRoot());
        auto toAccountAfter = tang::loadAccount(ls, destination);
        // check that the target account didn't change
