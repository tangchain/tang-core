...
InflationOpFrame::doApply(Application& app, AbstractLedgerState& ls)
{
    ...
    */
    ...
    for (auto const& w : winners)
    {
        ...
        {
            auto winner = tang::loadAccountWithoutRecord(ls, w.accountID);
            if (winner)
            {
        ...
        auto winner = tang::loadAccount(ls, w.accountID);
        if (winner)
        {
