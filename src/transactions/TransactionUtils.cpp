                            ...
                            LedgerStateEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ls, &sellerID]() {
        auto account = tang::loadAccount(ls, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ls, &sellerID](Asset const& asset) {
        auto trust = tang::loadTrustLine(ls, sellerID, asset);
        if (!trust)
        {
           ...
           int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!tang::addBalance(newBalance, delta))
        {
            return false;
        }
    ...
    else if (entry.current().data.type() == TRUSTLINE)
    {
        ...
        if (!tang::addBalance(newBalance, delta, tl.limit))
        {
            return false;
        }
        if (header.current().ledgerVersion >= 10)
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = tang::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = tang::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
