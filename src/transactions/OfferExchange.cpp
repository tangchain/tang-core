           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    {
        auto accountB = tang::loadAccountWithoutRecord(ls, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        ...
        auto accountB = tang::loadAccount(ls, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = tang::loadAccount(lsInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                tang::loadTrustLine(lsInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                tang::loadTrustLine(lsInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (!tang::loadAccountWithoutRecord(ls, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = tang::loadAccount(ls, accountBID);
    }
                                   ...
                                   : CrossOfferResult::eOfferPartial;
    {
        ...
        {
            sellingWheatOffer.erase();
            accountB = tang::loadAccount(lsInner, accountBID);
