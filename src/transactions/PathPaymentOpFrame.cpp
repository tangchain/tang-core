...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerState& ls)
{
    ...
    if (ls.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)tang::loadAccountWithoutRecord(ls, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!tang::loadAccountWithoutRecord(ls, mPathPayment.destination))
        {
            app.getMetrics()
    ...
    {
        auto destination = tang::loadAccount(ls, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                tang::loadAccountWithoutRecord(ls, getIssuer(curB));
            if (!issuer)
            {
                app.getMetrics()
        ...
        auto destLine =
            tang::loadTrustLine(ls, mPathPayment.destination, curB);
        if (!destLine)
        {
            app.getMetrics()
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!tang::loadAccountWithoutRecord(ls, getIssuer(curA)))
            {
                app.getMetrics()
                    .NewMeter({"op-path-payment", "failure", "no-issuer"},
                              "operation")
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = tang::loadAccount(ls, getSourceID());
            if (!sourceAccount)
            {
                app.getMetrics()
