                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = tang::loadAccount(ls, accountID);
        if (res)
        {
    ...
    {
        return tang::loadAccount(ls, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerState& ls)
{
    ...
    if (fee > 0)
    {
        ...
        tang::addBalance(acc.balance, -fee);
...
{
    auto account = tang::loadAccount(ls, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = tang::addNumEntries(header, account, -1);
        assert(removed);
