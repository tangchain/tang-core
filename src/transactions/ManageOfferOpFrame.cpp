                                    ...
                                    AbstractLedgerState& lsOuter)
{
    ...
    if (sheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = tang::loadAccount(ls, getIssuer(sheep));
        if (!issuer)
        {
            metrics
    ...
    if (wheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = tang::loadAccount(ls, getIssuer(wheat));
        if (!issuer)
        {
            metrics
...
ManageOfferOpFrame::doApply(Application& app, AbstractLedgerState& lsOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = tang::loadOffer(ls, getSourceID(), offerID);
        if (!sellSheepOffer)
        {
            app.getMetrics()
    ...
    if (mManageOffer.amount > 0)
    {
        ...
        if (header.current().ledgerVersion >= 10)
        {
            ...
            {
                auto sourceAccount =
                    tang::loadAccountWithoutRecord(ls, getSourceID());
