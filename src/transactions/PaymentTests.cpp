...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("send XLM to an existing account")
    {
        for_all_versions(*app, [&] {
            ...
            {
                LedgerState ls(app->getLedgerStateRoot());
                auto rootAccount2 = tang::loadAccount(ls, root);
                auto rootAcc2 = rootAccount2.current().data.account();
                auto a1Account2 = tang::loadAccount(ls, a1);
    ...
    SECTION("rescue account (was below reserve)")
    {
        for_all_versions(*app, [&] {
            auto getMinBalance = [&] {
                LedgerState ls(app->getLedgerStateRoot());
                return tang::getMinBalance(ls.loadHeader(), 0);
            };
            ...
            for (auto const& data : withoutTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("without trustline")
                    {
                        ...
                        REQUIRE(!tang::loadTrustLine(ls, sendToSelf,
                                                        data.asset));
                    }
                }
            }
            ...
            for (auto const& data : withTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("with trustline and 0 balance")
                    {
                        ...
                        auto trust =
                            tang::loadTrustLine(ls, sendToSelf, data.asset);
                        REQUIRE(trust);
