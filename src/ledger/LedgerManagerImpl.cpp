...
LedgerManagerImpl::startNewLedger(LedgerHeader genesisLedger)
{
    ...
    ls.loadHeader().current() = genesisLedger;
    ...
    function<void(asio::error_code const& ec)> handler)
{
    ...
    else
    {
        ...
        {
            LedgerState ls(mApp.getLedgerStateRoot());
            advanceLedgerPointers(ls.loadHeader().current());
        }
    }
}
...
LedgerManagerImpl::valueExternalized(LedgerCloseData const& ledgerData)
{
    CLOG(INFO, "Ledger")
        ...
        << ", sv: " << tangValueToString(ledgerData.getValue()) << "]";
...
LedgerManagerImpl::applyBufferedLedgers()
{
    ...
    mApp.postOnMainThreadWithDelay([&] {
        ...
        CLOG(INFO, "Ledger")
            ...
            << ", sv: " << tangValueToString(lcd.getValue()) << "]";
        closeLedger(lcd);
...
LedgerManagerImpl::closeLedger(LedgerCloseData const& ledgerData)
{
    ...
    ls.loadHeader().current().txSetResultHash =
    ...
    for (size_t i = 0; i < sv.upgrades.size(); i++)
    {
        ...
        try
        {
            LedgerState lsUpgrade(ls);
            Upgrades::applyTo(lupgrade, lsUpgrade);

            auto ledgerSeq = lsUpgrade.loadHeader().current().ledgerSeq;
