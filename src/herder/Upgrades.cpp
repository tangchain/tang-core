...
void
save(Archive& ar, tang::Upgrades::UpgradeParameters const& p)
{
    ar(make_nvp("time", tang::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, tang::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = tang::VirtualClock::from_time_t(t);
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!tang::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerState& ls)
{
    ...
    {
        auto trust = tang::loadTrustLineWithoutRecord(ls, accountID, asset);
...
prepareLiabilities(AbstractLedgerState& ls, LedgerStateHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = tang::loadAccount(ls, accountOffers.first);
        if (!accountEntry)
        {
        ...
        for (auto& offerEntry : accountOffers.second)
        {
            ...
            {
                tang::addNumEntries(header, accountEntry, -1);
            }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    tang::loadTrustLine(ls, accountOffers.first, asset);
                int64_t deltaSelling =
