...
The `get` and `put` commands define subprocesses that `tang-core` will
...
called `s3tang` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3tang]
get="curl http://history.tang.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.tang.org/{1}"
~~~~

In this example, `tang-core` will use the `curl` command to fetch history and
