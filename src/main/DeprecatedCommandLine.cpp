...
const struct option tang_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: tang-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time tang-core is run, SCP will "
          "start "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("tang-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", tang_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
