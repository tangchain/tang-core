...
CommandHandler::testAcc(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        ...
        auto acc = tang::loadAccount(ls, key.getPublicKey());
        if (acc)
        {
...
{
    retStr = "<b>Welcome to tang-core!</b><p>";
    ...
    retStr +=
        ...
        "Cursors are used by dependent services to tell tang - core which "
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the tang-core.cfg if you want this "
            "behavior";
    }
}
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the tang-core.cfg if you want this behavior";
    }
}
