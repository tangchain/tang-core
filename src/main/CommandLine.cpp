...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"tang-core.cfg"} : mConfigFile;
...
runForceSCP(CommandLineArgs const& args)
{
    ...
    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force SCP flag, so next time tang-core "
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"force-scp",
          "next time tang-core is run, SCP will start with "
         ...
         {"http-command", "send a command to local tang-core",
          runHttpCommand},
