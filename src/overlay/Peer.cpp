...
void
Peer::recvMessage(StellarMessage const& tangMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(tangMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (tangMsg.type() != HELLO) &&
        (tangMsg.type() != AUTH) && (tangMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << tangMsg.type() << " before completed handshake";
        drop();
        return;
    }

    assert(isAuthenticated() || tangMsg.type() == HELLO ||
           tangMsg.type() == AUTH || tangMsg.type() == ERROR_MSG);
